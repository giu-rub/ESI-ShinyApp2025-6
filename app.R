...
theme <- bs_theme(
  bootswatch   = "united",
  base_font    = font_google("PT Serif", wght = 400),
  heading_font = font_google("Fira Sans", wght = 700),
  primary      = "#E68059"
)
...
load_map_raster <- function(path = "data/df_esi_inner(withc_esi).csv") {
  ...
  if (
    length(lon_candidates) >= 1 &&
    length(lat_candidates) >= 1 &&
    ...
    r_xyz <- try(
      terra::rast(xyz_df, type = "xyz", crs = "EPSG:4326"),
      silent = TRUE
    )
  ...
  if (
    !inherits(r_guess, "try-error") &&
    all(c("l_esi", "w_esi", "c_esi") %in% names(r_guess))
  ) {
    return(r_guess)
  }
  
  warning(
    "Could not coerce CSV to SpatRaster. Using a dummy raster so the app can run.\n",
...
toolbox_page_ui <- function() {
  tagList(
    tags$head(
      tags$title("ESI Toolbox"),
      bslib::bs_theme_dependencies(theme),
      tags$link(rel = "stylesheet", href = "esi.css")
    ),
